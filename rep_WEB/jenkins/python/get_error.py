...
path = r'D:\TS_apiTest\rep_WEB\jenkins\errorLog\WEB_api_testreport_error.txt'
error=[]
...
def sendmessage(errorName,message):
    ...
    message = message
    if message == "0":
        msg='nice！预生产环境WEB接口测试构建完成，本次自动化测试没有发现异常！'
        phone=''
    else:
        msg='后台异常！！！预生产环境WEB接口测试构建完成，本次自动化测试有'+errorName+'共计'+message+'条用例异常，请下载报告分析'
        phone='+86-15926111515'
    String_textMsg = {
        "msgtype": "text",
        "text": {"content": msg},
        "at": {
