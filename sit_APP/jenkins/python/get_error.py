...
path = r'D:\TS_apiTest\sit_APP\jenkins\errorLog\APP_api_testreport_error.txt'
error=[]
...
def sendmessage(errorName,message):
    ...
    message = message
    if message == "0":
        msg='nice！测试环境APP接口测试构建完成，本次自动化测试没有发现异常！'
        phone=''
    else:
        msg='后台异常！！！测试环境APP接口测试构建完成，本次自动化测试有'+errorName+'共计'+message+'条用例异常，请下载报告分析'
